...
// Composant pour le ladder Global
const GlobalLadder = () => {
  ...
  // Fonction pour récupérer les données du ladder Global
  const fetchLadderData = async () => {
    setIsLoading(true);
    setError(null);
    
    try {
      const response = await axios.get(`${API_BASE_URL}/global-ladder-data`);
      
      if (response.data.success) {
        const ladderData = response.data;
        
        // Pour le global ladder, on utilise directement les données
        const playerData = ladderData.players || [];
  ...
  const refreshLadderData = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/refresh-global-ladder-data`);
    ...
    <div className="w-full py-8 bg-gray-900 text-gray-100 min-h-screen">
      ...
      <div className="max-w-1/2 mx-auto px-4 mb-8">
        <div className="text-3xl flex font-bold text-center items-center mb-6 text-blue-300">
          Global Ladder
          <Link to="/ladder/soloq" className="ml-4 text-base bg-gray-800 text-gray-300 px-4 py-2 rounded-lg hover:bg-gray-700 transition">
            Voir Solo Queue
          </Link>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gray-900 text-gray-300 border-b border-gray-700">
                ...
                <th className="px-4 py-3 text-center">Région</th>
                  ...
                  return (
                    <tr key={player.puuid || index} className="hover:bg-gray-700">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="text-sm font-medium text-gray-300">{player.region || "N/A"}</span>
                      </td>
                      <td className="px-4 py-3">
